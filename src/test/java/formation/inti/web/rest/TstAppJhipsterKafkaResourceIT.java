...
import formation.inti.config.KafkaProperties;
...
class TstAppJhipsterKafkaResourceIT {
    ...
    void setup() {
        ...
        TstAppJhipsterKafkaResource kafkaResource = new TstAppJhipsterKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/tst-app-jhipster-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/tst-app-jhipster-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
